...
void
AmclNode::imagePoseReceived(geometry_msgs::msg::PoseStamped::SharedPtr msg)
{
image_pose_rx_=*msg;
}
void
AmclNode::imagePoseReceived2(geometry_msgs::msg::PoseStamped::SharedPtr msg)
{
image_pose_rx_2=*msg;
  ...
  const pf_vector_t & pose)
{
  ...
  RCLCPP_DEBUG(
    ...
    angle_increment);
  RCLCPP_WARN(get_logger(),"laser_maxxxxxxxxxxxxxxxxxxxxxxxxx: %f",laser_max_range_);
  ...
  // pose from two cameras
  image_pose_ = create_subscription<geometry_msgs::msg::PoseStamped>(
    "pose_image", rclcpp::SystemDefaultsQoS(),
    std::bind(&AmclNode::imagePoseReceived, this, std::placeholders::_1));
  image_pose_2 = create_subscription<geometry_msgs::msg::PoseStamped>(
    "pose_image_nearcam", rclcpp::SystemDefaultsQoS(),
    std::bind(&AmclNode::imagePoseReceived2, this, std::placeholders::_1));


  lasers_[laser_index]->sensorUpdate(pf_, reinterpret_cast<nav2_amcl::LaserData *>(&ldata),image_pose_rx_,image_pose_rx_2);
  ...
  const std::vector<amcl_hyp_t> & hyps, const int & max_weight_hyp)
{
  ...
  // std::cout<<"diff "<<now().seconds()<<std::endl;
  // std::cout<<"diffnan "<<now().nanoseconds()<<std::endl;
  // std::cout<<"diffp "<<p->header.stamp.sec<<std::endl;
  // std::cout<<"diffnanp "<<p->header.stamp.nanosec<<std::endl;
  ...
  //   diff=(p->header.stamp.sec+(p->header.stamp.nanosec*1e-9))-now().nanoseconds()*1e-9;
  // std::cout<<"diff"<<diff<<std::endl; 0.04 secs avg
...
AmclNode::sendMapToOdomTransform(const tf2::TimePoint & transform_expiration)
{
  ...
  // double diff;
  // diff=(tmp_tf_stamped.header.stamp.sec+(tmp_tf_stamped.header.stamp.nanosec*1e-9))-now().nanoseconds()*1e-9;
